 ...
 * @LastEditTime: 2023-03-27 17:48:26
 * @Description: 
 */
...
const save = async (config,argv,yargs) => {
    const propData = yargs.wrap(Math.min(150, yargs.terminalWidth()))
        ...
        .parse(argv);
        const archiveName = typeof config['save'].archiveName === 'string' ? config['save'].archiveName : config['save'].archiveName({
            time : {
    ...
    try {
        ...
        childProcess.spawn('docker', [
                ...
                `${imagesName}:${version}`,
                ">",
                `${path.join(dockerDir, archiveName)}`
            ], {
            stdio: "inherit",
            shell: true,
        });
